...
/// cohort state and ORE's round account, so the caller cannot influence it.
#[derive(Accounts)]
pub struct Settle<'info> {
    #[account(mut)]
    pub cohort: AccountLoader<'info, Cohort>,
    /// CHECK: validated as the target ORE Round in `ore::read_round`.
...
pub fn handle_settle(ctx: Context<Settle>) -> Result<()> {
    ...
    let rng = match ore::read_round(&ctx.accounts.ore_round.to_account_info(), target)? {
        RoundState::Present(Some(rng)) => rng,
        _ => return err!(LockedInError::RoundNotRevealed),
    };
    let winning_square = ore::winning_square(rng);
    let motherlode = ore::hit_motherlode(rng);
    let picked = cohort.slots[i].square;

    let mut tier = if motherlode {
        TIER_LEGENDARY
    } else if picked == winning_square {
        TIER_RARE
    } else {
        TIER_COMMON
    };
    ...
    msg!(
        "Locked In settle: round {} winning_square {} picked {} motherlode {} tier {} amount {}",
        target,
        winning_square,
        picked,
        motherlode,
...
/// Moves a pick to a new round only when its target round is over and unusable: closed after expiry,
/// or finished without usable entropy. A round that is merely not revealed yet cannot be skipped.
#[derive(Accounts)]
...
pub fn handle_retarget(ctx: Context<Retarget>, user: Pubkey) -> Result<()> {
    ...
    // The board has moved past the target, so ORE has finished with it.
    require!(
        current_round > slot.target_round,
        LockedInError::RetargetNotAllowed
    );
    let unusable = match ore::read_round(&ctx.accounts.ore_round.to_account_info(), slot.target_round)? {
        RoundState::Missing => true,
        RoundState::Present(rng) => rng.is_none(),
    };
    require!(unusable, LockedInError::RetargetNotAllowed);
